...
router.get('sendMessage', function* () {
    ...
    if (phoneRegexp.test(phone)) { } //TODO:代码被注释 原因不明
    var code = Math.random().toString().substring(3, 9);

    yield db.PhoneCode.create({ phone: this.request.query.phone, code: code });
...
router.loginFunction = function* () {
    ...
    this.required("type", "phone", "code");
    if (this.request.query.type == "phone") {
        this.required("code");
        var phoneCode = yield db.PhoneCode.find({ where: { phone: this.request.query.phone, code: this.request.query.code } });
        //TODO:code是否需要建立索引还是需要通过建立数组来添加code
        if (phoneCode != null) {
            yield db.PhoneCode.destroy({ where: { id: phoneCode.id } });
            if (((Date.now() - phoneCode.id / 1000) / (1000 * 60)) < 10) { } else {
                this.message("验证码已过期");
                return;
            }
        } else {
...
router.get('logout', function* () {
    // yield db.Token.findOneAndRemove({
    //     token: this.cookies.get("token")
    // });
    var token = yield db.Token.findOne({ where: { token: this.cookies.get("token") } });
    yield token.destroy({ where: { token: this.cookies.get("token") } });
