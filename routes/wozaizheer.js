...
router.get('public', function* () {
    ...
    if (this.query != undefined && this.query.index != undefined) {
        if (this.query.index.charAt(0) == ">") { //shang
            bubblemap = yield db.Bubblemap.findAll({ where: { map: this.query.map, id: { gt: this.query.index.substr(1) } }, limit: 25, order: 'id ASC' });
            if (bubblemap.length > 1)
                bubblemap.reverse();
        } else if (this.query.index.charAt(0) == "<") { //xia
            bubblemap = yield db.Bubblemap.findAll({ where: { map: this.query.map, id: { lt: this.query.index.substr(1) } }, limit: 25, order: 'id DESC' });
        }

    } else {
        bubblemap = yield db.Bubblemap.findAll({ where: { map: this.query.map }, limit: 25, order: 'id DESC' });
    }
