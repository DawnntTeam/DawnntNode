...
router.get('markRead', function* () {
    ...
    if (this.query != undefined && this.query.id != undefined) {
        var notify = yield db.Notify.findOne({ where: { targetUser: this.user.id, id: this.query.id } });
        notify.update({ state: true });
        notify.state = true;
        this.body = notify;
    } else {
        this.throw403("id");
...
router.get('markSeveralRead', function* () {
    ...
    if (this.query != undefined && this.query.id != undefined) {
        ...
        yield db.Notify.update({ state: true }, { where: { id: temp } });
        this.body = yield db.Notify.findAll({ where: { id: temp } });
    } else {
        this.throw403("id");
    }
});
...
router.get('show', function* () {
    ...
    this.checkAuth();
    var where = { targetUser: this.user.id };
    var order = "id DESC";
    ...
    if (this.query != undefined) {
        ...
        if (this.query.index != undefined) {
            if (this.query.index.charAt(0) == ">") { //shang
                where.id = { gt: this.query.index.substr(1) };
                order = "id ASC";
            } else { //下一页 next
                where.id = { lt: this.query.index.substr(1) };
            }
        }
    }
    var date = { notify: [] };
    var notify = yield db.Notify.findAll({
        ...
        population: [
            { model: "user", col: "user" },
            { model: "user", col: "targetUser" },
            { model: "status", col: "status" },
            { model: "comment", col: "comment" },
            { model: "comment", col: "reply" }
        ],
...
router.unread = function* (targetUser) {
    var notify = yield db.Notify.findAll({ where: { state: false, targetUser: targetUser } });
    var systemNum = 0;
    var friendsNum = 0;
    if (notify) {
    ...
    return { systemNum: systemNum, friendsNum: friendsNum };
};
