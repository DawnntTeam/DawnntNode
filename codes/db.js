        ...
        if (Array.isArray(options.population)) {
            ...
            options.population.forEach(function(population) {
                ...
                if (Array.isArray(attributes)) {
                    var a = model.findAll(findOptions).then(function(result) {
...
exprots.User = sequelize.define('user', {
    ...
    sex: { type: Sequelize.ENUM, values: ['male', 'female', 'neutral'] },
    phone: { type: Sequelize.STRING },
    email: { type: Sequelize.STRING },
    head: { type: Sequelize.STRING },
    birthday: { type: Sequelize.DATE, defaultValue: Date.now },
    desc: { type: Sequelize.STRING },
    location: { type: Sequelize.STRING },
    albumCover: { type: Sequelize.STRING }
}, {
        getterMethods: {
            create_at: function() {
...
exprots.Notify = sequelize.define('notify', {
    ...
    user: { type: Sequelize.BIGINT },
    targetUser: { type: Sequelize.BIGINT },
    status: { type: Sequelize.BIGINT },
    comment: { type: Sequelize.BIGINT },
    reply: { type: Sequelize.BIGINT },
    option: { type: Sequelize.JSONB }
}, {
        getterMethods: {
            create_at: function() {
                return new Date(this.id / 1000)
